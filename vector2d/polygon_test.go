...
func Test_LineIsPolygonConvex(t *testing.T) {
	s := NewPolygon(0, 0, 100, 100)
	s.Translate(1, 1)

	assert.Equal(t, s.Matrix[0][0], 1.0)
	assert.Equal(t, s.Matrix[0][1], 1.0)
	assert.Equal(t, s.Matrix[1][0], 101.0)
	assert.Equal(t, s.Matrix[1][1], 101.0)
}
